...
namespace _04._Raw_Data
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var result = cars.Where(x => x.Cargo.CargoType == typePerCargo && x.Cargo.CargoWeight < 1000).ToList();
            ...
            {
                var result = cars.Where(x => x.Cargo.CargoType == typePerCargo && x.Engine.EnginePower > 250).ToList();
                foreach (var car in result)
                {
                    Console.WriteLine(car.Model);
                }
